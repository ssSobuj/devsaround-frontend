import Image from "next/image";

type Props = {
  data: { image: string; name: string; position: string; description: string };
};

export default function TeamCard({ data }: Props) {
  const { image, name, position, description } = data || {};
  return (
    <div
      className={`py-[40px] px-[35px] rounded-[45px] border border-secondary shadow-[0px_5px_2px_0px_#000000] flex flex-col gap-7`}
    >
      <div className="flex items-end gap-5 relative">
        <Image src={image} width={105} height={102} alt={name} />

        <div className="flex flex-col justify-end items-start w-full h-full">
          <h4 className="font-medium">{name}</h4>
          <p className="text-sm">{position}</p>
        </div>
        <div className="absolute top-0 right-0 bg-foreground rounded-full w-[34px] h-[34px] flex items-center justify-center">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="17"
            height="17"
            viewBox="0 0 17 17"
            fill="none"
          >
            <path
              d="M0.317757 17H3.81308V5.68438H0.317757V17Z"
              fill="#B9FF66"
            />
            <path
              d="M0 2.07188C0 3.1875 0.900311 4.09062 2.06542 4.09062C3.17757 4.09062 4.07788 3.1875 4.07788 2.07188C4.07788 0.95625 3.17757 0 2.06542 0C0.900311 0 0 0.95625 0 2.07188Z"
              fill="#B9FF66"
            />
            <path
              d="M13.4517 17H17V10.7844C17 7.75625 16.3115 5.36562 12.7632 5.36562C11.0685 5.36562 9.90343 6.32188 9.42679 7.225H9.37383V5.68438H6.03738V17H9.53271V11.4219C9.53271 9.93437 9.79751 8.5 11.6511 8.5C13.4517 8.5 13.4517 10.2 13.4517 11.475V17Z"
              fill="#B9FF66"
            />
          </svg>
        </div>
      </div>
      <div className="w-full border-t border-one border-foreground"></div>
      <p>{description}</p>
    </div>
  );
}
